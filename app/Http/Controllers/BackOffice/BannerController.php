...
class BannerController extends BackOfficeController
{
	public function datatables(){
		...
		$datatables = datatables($data)
			->addColumn('_buttons',function($row){
				$editurl = route('backoffice.banner.edit',$row->id);
	...
	public function index(){
		return view('backoffice.banner.list');
	}
	public function edit($id){
		$data = User::firstWhere('id',$id);
		$data->password='';
		return view('backoffice.banner.form',['title'=>"Edit - {$data->name}",'data'=>$data]);
	}
	public function insert(){
		return view('backoffice.banner.form',['title'=>'Insert','data'=>[
			'id'=>'',
			'name'=>'',
			'email'=>'',
			'password'=>'',
