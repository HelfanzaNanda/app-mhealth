...
class UsersController extends BackOfficeController
{
	public function datatables(){
		...
		$datatables = datatables($data)
			->addColumn('_buttons',function($row){
				$editurl = route('backoffice.users.edit',$row->id);
	...
	public function edit($id){
		...
		$data->password='';
		return view('backoffice.users.form',['title'=>"Edit - {$data->name}",'data'=>$data]);
	}
	public function insert(){
		return view('backoffice.users.form',['title'=>'Insert','data'=>[
			'id'=>'',
			'name'=>'',
			'email'=>'',
			'password'=>'',
